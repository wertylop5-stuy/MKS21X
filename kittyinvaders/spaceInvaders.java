...
//enemies.get(enemies.size()/2).toggleDebug();
    ...
    private byte mDirection;	//0 right, 1 left
    private byte mMovingDown;	//0 false, 1 true
	...
	public void keepInBounds() {
	    ...
	    //System.out.println(Dist(getX(), mDownPoint, getX(), getY()));
	    if (mMovingDown == 1) {
	    	if (Dist(getX(), mDownPoint, getX(), getY()) >= 100.0f) {
				toggleDirection();
				toggleDown();
				mDownPoint = -1.0f;
			}
	    	...
	    	if (getX() > width - getWidth()/2 && mDirection == 0) {
				setX(width - getWidth()/2 -1);
				toggleDown();
				mDownPoint = getY();
			}
			else if (getX() < getWidth() / 2 && mDirection == 1) {
				setX(getWidth() / 2 + 1);
				toggleDown();
				mDownPoint = getY();
			}
			else {
				if (mDirection == 1) moveLeft();
				else moveRight();
			}
	    }
	    if (getY() > (height - getHeight() / 2)) deleteMe();
	}
	...
	private void toggleDirection() {
		...
		mDirection = (byte)(mDirection ^ 0b0001);
	}
	private void toggleDown() { 
		//System.out.println("down toggled");
		mMovingDown = (byte)(mMovingDown ^ 0b0001);
	}
